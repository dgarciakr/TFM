...
class Reward:
    ...
    def __init__(self, env_state):
        ...
        self.msgs_matrix = env_state["msgs_matrix"]
        self.msgs_matrix = self.msgs_matrix / np.max(self.msgs_matrix)  
    ...
    # Reward base on the number of messages communicated between two 
    ...
    def get_reward(self, state):
        ...
        for src in range(0, self.P):
            ...
            n_inter += np.dot(tmp_state, self.msgs_matrix[src])
            
        """ Computation: 
        for src in range(0, self.P):
            for dst in range(0, self.P):
                if self.msgs_matrix[src,dst] != 0: # Different node
                    n_msgs = self.msgs_matrix[src,dst]
        ...
        #reward = np.sqrt(n_inter**2 + (self.P * oversubs)**2)
